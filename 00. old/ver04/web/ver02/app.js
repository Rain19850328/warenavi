// r14: Move flow with '이동위치 선택' (close dialog -> pick on rack -> reopen)
const API_BASE = '/api';
...
/* === 이동 다이얼로그: '이동위치 선택' 버튼으로 닫았다가 재오픈 === */
...
function setMoveTarget(ci, cellEl){
  ...
  if(!ci){ return; }
  // prevent selecting source cell as target
  ...
  // if user pressed "이동위치 선택" and dialog was closed, reopen now
  if (MOVE.reopenAfterPick){
    MOVE.reopenAfterPick = false;
    openMove(true); // reopen without resetting state
  }
}
...
async function openMove(preserve=false){
  ...
  if (pickBtn){
    pickBtn.onclick = (e)=>{
      ...
      closeDialog(dlg); // close so rack is clickable
      // user now clicks a target cell; setMoveTarget will reopen dialog
    };
  }
