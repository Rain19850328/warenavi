// r11: Move by clicking a target cell on the rack (no text input)
const API_BASE = '/api';
...
async function postJSON(url, body) {
  ...
  let data = {};
  try { data = await r.json(); } catch (_) { /* 204/빈응답 안전 */ }
...
/* ---------- Move picking state ---------- */
const MOVE = { active:false, sourceCode:null, sku:null, maxQty:0, targetCode:null };
  ...
  for (const bay of bays) {
    ...
    for (let level=CONFIG.levels; level>=1; level--){
      ...
      if (ci) {
        ...
        // restore move target highlight if applicable
...
function showDetail(ci) {
  ...
  for (const it of (ci.items||[])) {
    ...
    tr.innerHTML = `<td>${it.sku}</td><td>${it.name||''}</td><td style="text-align:center">${(it.qty??0).toLocaleString('ko-KR')}</td>`;
  ...
  if(!dlg){
    ...
    dlg.innerHTML = `
      <form method="dialog" class="dialog">
        ...
        <div class="row">
          <div>대상: <strong id="moveTargetCode">-</strong> <span class="muted">(현황판에서 클릭해 선택)</span></div>
        </div>
        <div class="row">
...
function setMoveTarget(ci, cellEl){
  ...
  // can't choose level header cells
  if(!ci){ return; }
  // prevent selecting source cell as target (allow? -> here disallow to avoid no-op)
  if (MOVE.sourceCode && MOVE.sourceCode === ci.code) {
    // still show highlight if you want to visualize, but ignore set
    return;
  }
...
async function openMove(){
  ...
  if(title)  title.textContent = `이동(${CURRENT_CELL.code})`;
  if(meta)   meta.textContent  = `FROM ${CURRENT_CELL.code} · SKU: ${item.sku} · 보유: ${item.qty}개`;
  if(codeEl) codeEl.textContent = '-';
  if(qtyEl){ qtyEl.min = 1; qtyEl.max = Math.max(1, item.qty); qtyEl.value = item.qty; }
  ...
  dlg.querySelector('#moveOk').onclick = async (e)=>{
    e.preventDefault();
    if(!MOVE.targetCode) return alert('대상 위치를 현황판에서 선택하세요.');
