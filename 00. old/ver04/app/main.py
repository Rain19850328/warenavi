...
class MySQLProvider(DataProvider):
  ...
  def move(self, from_rack: str, to_rack: str, item_code: str, qty: int) -> None:
    if qty <= 0: raise ValueError("수량은 1 이상이어야 합니다.")
    if from_rack == to_rack: return  # 동일 위치면 무시
    mode = self._schema_mode
    try:
      with self.engine.begin() as conn:
        # 재고 확인
...
class FakeProvider(DataProvider):
  ...
  def _apply_delta(self, rack_code: str, item_code: str, delta_qty: int):
    ...
    # find existing
    idx = None
    for i, (sku, nm, q, vol) in enumerate(ci.items):
      if sku == code:
        idx = i; break
    if delta_qty > 0:
      if idx is None:
        ci.items.append((code, name, delta_qty, used_delta))
      else:
        sku, nm, q, vol = ci.items[idx]
        ci.items[idx] = (sku, nm, q + delta_qty, vol + used_delta)
      ...
      if new_q == 0:
        ...
        ci.items[idx] = (sku, nm, new_q, new_vol)
...
# ---------- FastAPI ----------
app = FastAPI(title="Warehouse PWA API", version="1.1.0")
app.add_middleware(
  CORSMiddleware,
  allow_origins=["*"],
  allow_credentials=False,  # 쿠키 미사용이면 False 권장
  allow_methods=["*"],
  allow_headers=["*"],
)
