...
function ensureSearchDialog() {
  ...
  function renderList(items) {
    ...
    for (const it of (items || [])) {
      ...
      row.style.gridTemplateColumns = '80px minmax(0, 1fr) 40px'; // [SKU/LOC] | [상품명] | [수량]
...
async function init() {
  ...
  document.querySelector('#btnFooterGoLocation')?.addEventListener('click', async ()=>{
    const code = (prompt('로케이션 코드 입력 (예: SR1-05-02, SR2-A-01, SR-01-01-01)')||'').trim();
    if (!code) return;
    try {
      const data = await getJSON(`${API_BASE}/search_racks?q=${encodeURIComponent(code)}&limit=1`);
      const r = (data.results||[])[0];
      if (!r) { alert('일치하는 위치를 찾을 수 없습니다.'); return; }
